...
def main():
    ...
    assets = {'version': '13.0.0', 'files': {}}


    for f in os.listdir(dir_in):
        if f == 'cdk.out' or f == 'tree.json' or f == 'manifest.json':
            continue

        pack = "zip" if os.path.isdir(dir_in + "/" + f) else "file"
        key = BUCKET_NAME + "/" + VERSION + "/" + f
        name = f[6:] if f.startswith('asset.') else f
        name = name[0:-4] if name.endswith('.zip') else name
        assets['files'].update({name: {
            "source": {
                "path": f,
                "packaging": pack
            },
    ...
    with open(dir_in + '/SuperResolutionStack.assets.json', 'w') as json_file:
